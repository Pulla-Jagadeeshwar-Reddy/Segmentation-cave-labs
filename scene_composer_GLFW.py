...
SH_C0 = 0.28209479177387814  # Y_0^0 spherical-harmonic constant
...
def write_gaussian_ply(path: str, mean: np.ndarray, color: np.ndarray,
                        opacity: np.ndarray, cov3d: np.ndarray):
    """Write (mean, color, opacity, cov3d) as a standard 3DGS binary .ply."""
    N = len(mean)
    ...
    opac_logit = np.log(opac / (1.0 - opac)).astype(np.float32).reshape(-1, 1)

    f_dc = ((color.astype(np.float64) - 0.5) / SH_C0).astype(np.float32)
    f_rest = np.zeros((N, 45), dtype=np.float32)   # no higher-order SH fit
    ...
    attrs = np.concatenate(
        [mean.astype(np.float32), normals, f_dc, f_rest,
         opac_logit, log_scale, quat], axis=1)
...
def reposed_object_gaussians(job: "SplatJob", R_total: np.ndarray,
    ...
    """Re-poses job.gauss by the viewer's accumulated transform, returns
    (mean, color, opacity, cov3d) arrays — does NOT write anything.

    mean:   rotate about base_centroid, then place at current_centroid
              mean' = R_total @ (mean - base_centroid) + current_centroid
    cov3d:  a rotation of the point rotates its covariance the same way
              Sigma' = R_total @ Sigma @ R_total.T
    color / opacity are unaffected by rigid motion.
    """
    g = job.gauss
    mean  = g["mean"].astype(np.float64)
    cov3d = g["cov3d"].astype(np.float64)
    ...
    return mean_t, g["color"].astype(np.float64), g["opacity"].astype(np.float64), cov3d_t
...
def export_object_as_gaussian_splat(job: "SplatJob", R_total: np.ndarray,
    ...
    mean_t, color, opacity, cov3d_t = reposed_object_gaussians(
        job, R_total, base_centroid, current_centroid)
    write_gaussian_ply(out_path, mean_t, color, opacity, cov3d_t)
    return len(mean_t)
...
def room_points_as_gaussians(room_pcd: "o3d.geometry.PointCloud",
    ...
    """Turns plain room points into tiny ISOTROPIC Gaussians so the whole
    scene can live in one splat file. There's no PCA/neighborhood fit here
    (the room was never Gaussian-fit) — each point just becomes a small
    round blob. Size is adaptive: each blob's radius is set from that
    point's own nearest-neighbor distance (via cKDTree), so dense areas get
    small tight splats and sparse areas get bigger ones instead of either
    swiss-cheesing or ballooning into fog. A tiny floor (relative to the
    room's bounding box) guards against zero-size splats at duplicate
    points.

    Returns a dict shaped like SplatJob.gauss: mean, color, opacity, cov3d.
    """
...
def export_scene_as_gaussian_splat(room_pcd: "o3d.geometry.PointCloud",
    ...
    """Writes ONE .ply containing the whole scene: the room as small
    isotropic Gaussians plus the object as its real re-posed fitted
    Gaussians, so a splat renderer opens a single file for everything."""
    room_g = room_points_as_gaussians(room_pcd, k=room_k)
    obj_mean, obj_color, obj_opacity, obj_cov3d = reposed_object_gaussians(
        job, R_total, base_centroid, current_centroid)

    mean    = np.vstack([room_g["mean"],    obj_mean])
    color   = np.vstack([room_g["color"],   obj_color])
    opacity = np.concatenate([room_g["opacity"], obj_opacity])
    cov3d   = np.vstack([room_g["cov3d"],   obj_cov3d])

    write_gaussian_ply(out_path, mean, color, opacity, cov3d)
    return len(room_g["mean"]), len(obj_mean)
...
class SplatJob:
    ...
    def __init__(self):
        ...
        self.error    = None
        self.gauss    = None   # raw dict from estimate_gaussians_fast
        self.splat_pcd = None  # o3d.PointCloud — density-sampled splat cloud
        self.centroid  = None

    def run(self, obj_pcd, k=16, scale=1.0):
        try:
...
def _extract_ground(pcd_full: o3d.geometry.PointCloud,
    ...
    """
    Recover the ground points that remove_ground() stripped out.
    Strategy: build a KD-tree on pcd_no_ground, then for every point in
    pcd_full find its nearest neighbour.  Points whose nearest-neighbour
    distance is larger than a tiny epsilon are NOT in pcd_no_ground —
    those are the ground points.
    This is robust regardless of what remove_ground() does internally.
    """
    pts_full = np.asarray(pcd_full.points)
    pts_kept = np.asarray(pcd_no_ground.points)

    if len(pts_kept) == 0 or len(pts_full) == 0:
        # Edge case: nothing was removed or nothing remains
...
class CombinedScene:
    def __init__(self, room_pcd, job: SplatJob):
    ...
    def _export_gaussians(self, vis=None):
        ...
        try:
            ...
            print("[export] Standard 3DGS layout (x,y,z / f_dc / opacity / "
                  ...
                  "Room points are small round splats; the object keeps its "
                  "real fitted shape.")
        except Exception:
            import traceback
            traceback.print_exc()
...
def main():
    ...
    pcd_full = pcg.load_pcd(filepath)

    # ── Extract ground before clustering so we can put it back later ──────
    # remove_ground() returns only the non-ground points; we recover the
    # ground by diffing the full cloud against the returned subset so it
    # can be merged back into room_pcd before the viewer and export.
    pcd = pcg.remove_ground(pcd_full)
    ground_pcd = _extract_ground(pcd_full, pcd)
    print(f"[ground] {len(ground_pcd.points):,} floor points will be restored to the scene.")
    ...
    def on_continue(pcd, labels, n_clusters, cid):
        ...
        # Merge the floor back into the room cloud so the viewer and
        # the export both include the ground plane.
        ...
        scene = CombinedScene(room_pcd, job)
        scene.run()
