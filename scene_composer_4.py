...
def split_room_and_objects(pcd, labels, cids):
    """Returns (room_pcd_without_objects, object_pcd, obj_mask).

    `cids` may be a single int or any iterable of cluster IDs — all of them
    become ONE combined object (requirement 7: multiple selected clusters
    are treated as a single movable entity, no other code needs to change).
    """
    if isinstance(cids, (int, np.integer)):
        cids = {int(cids)}
    else:
    ...
    obj_mask  = np.isin(labels, list(cids))
    room_mask = ~obj_mask
...
def reposed_object_gaussians(job: "SplatJob", R_total: np.ndarray,
    ...
    if job.orig is not None:
        # ── preserved path: only mean + rotation change ─────────────────
        rot_t = _rotate_quat_wxyz(job.orig["rot_raw"], R_total)
        return (mean_t.astype(np.float32), job.orig["f_dc"], job.orig["f_rest"],
    ...
    return encode_linear_to_raw(mean_t, job.gauss["color"].astype(np.float64),
...
def export_object_as_gaussian_splat(job: "SplatJob", R_total: np.ndarray,
    ...
    mean_t, f_dc, f_rest, opac_logit, log_scale, rot_t = reposed_object_gaussians(
        job, R_total, base_centroid, current_centroid)
...
class CombinedScene:
    def __init__(self, room_pcd, job: "SplatJob" = None, src: "SourceSplat" = None,
                 room_src_idx: np.ndarray = None,
                 nonground_pcd: "o3d.geometry.PointCloud" = None,
                 nonground_src_idx: np.ndarray = None,
                 ground_pcd: "o3d.geometry.PointCloud" = None,
                 ground_src_idx: np.ndarray = None):
        """
        `job` is now OPTIONAL — pass None to open the viewer with no object
        selected yet (the new default startup path). `nonground_pcd` /
        `nonground_src_idx` / `ground_pcd` / `ground_src_idx` are the same
        pieces `main()` used to compute and hand straight to
        split_room_and_object(s) — they now live here because selection
        happens on-demand, inside the running viewer, instead of once in
        main() before the viewer ever opens.
        """
        ...
        # state needed to (re)run clustering/selection from inside the viewer
        self.nonground_pcd     = nonground_pcd
        self.nonground_src_idx = nonground_src_idx
        self.ground_pcd        = ground_pcd
        self.ground_src_idx    = ground_src_idx
        self.mode = "normal"          # "normal" | "selecting" | "refining"
        self._selection_requested = False   # set by the S key callback, consumed in run()
        self._cluster_cache = None    # (pcd, labels, n_clusters, palette) — computed once, reused
        ...
        self._open_normal_window()

    # ── window management ───────────────────────────────────────────────────
    def _open_normal_window(self):
        """(Re)creates the standard VisualizerWithKeyCallback window showing
        self.room_pcd. Used at startup AND after Selection Mode confirms."""
        ...
        if self.obj_geom is not None:
            # Re-attach an already-fitted object (e.g. after a second
            # selection session) so it isn't lost across the window swap.
            self.vis.add_geometry(self.obj_geom)
            if getattr(self, "bbox_geom", None) is not None:
    ...
    def _export_gaussians(self, vis=None):
        if self.job is None or self.obj_geom is None or self.job.gauss is None:
            print("[export] No object selected yet — press S to select one, "
                  "or wait for splatting to finish, then press E again.")
            return False
    ...
    def _export_object_only(self, vis=None):
        if self.job is None or self.obj_geom is None or self.job.gauss is None:
            print("[export] No object selected yet — press S to select one, "
                  "or wait for splatting to finish, then press X again.")
            return False
        out_dir = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "splat_exports")
        os.makedirs(out_dir, exist_ok=True)
    ...
    def _register_keys(self):
        ...
        self.vis.register_key_callback(ord("S"), self._request_selection_mode)
    ...
    # ════════════════════════════════════════════════════════════════════
    # Selection Mode — triggered by "S". Reuses pcg.cluster_dbscan /
    # pcg.make_palette / pcg.build_display_pcd (same functions the old
    # show_labeled_cluster_overview used) and the same shift-click
    # convention, just non-blocking (a manual poll loop instead of
    # vis.run()) so we can recolor live as clusters are toggled.
    # ════════════════════════════════════════════════════════════════════
    def _request_selection_mode(self, vis=None):
        """Key callback for 'S'. Deliberately does almost nothing — it only
        sets a flag. The actual window destroy/create work happens in
        run(), safely OUTSIDE of poll_events()'s call stack. Doing that
        work here (inside the callback) means destroying/recreating the
        window while Open3D is still inside its own poll_events() call for
        THIS window — that re-entrancy is what was causing Selection Mode
        to fail to open / hang."""
        if self.mode != "normal":
            return False
        self._selection_requested = True
        return False

    def _run_selection_workflow(self):
        """The actual Selection Mode implementation. Only ever called from
        run(), after self.vis.poll_events()/update_renderer() have already
        returned for the frame — i.e. never from inside a GLFW callback."""
        self.mode = "selecting"
        print("[select] Entering Selection Mode …")

        # Cluster once, cache for any later selection session.
        if self._cluster_cache is None:
            print("[select] Clustering (DBSCAN) …")
            labels, n_clusters, eps = pcg.cluster_dbscan(self.nonground_pcd)
            palette = pcg.make_palette(n_clusters)
            self._cluster_cache = (labels, n_clusters, palette)
            print(f"[select] {n_clusters} clusters found (eps={eps:.4f}).")
        labels, n_clusters, palette = self._cluster_cache

        # Swap windows: destroy the normal viewer, open ONE editing window
        # showing the cluster-colored scene. Never two windows at once.
        self.vis.destroy_window()

        # NOTE: VisualizerWithEditing's shift-click picking is only reliable
        # when driven by its own blocking run() — the same call
        # show_labeled_cluster_overview() already uses successfully.
        # Manually pumping poll_events()/update_renderer() ourselves (the
        # previous version of this method) left the picking machinery
        # half-initialized, which is why the window came up blank and
        # Windows reported it as "Not Responding". So picking now happens
        # in one blocking pass — Open3D draws its own small marker at every
        # shift-clicked point automatically, giving live feedback during
        # the click itself even though the whole cluster isn't recolored
        # until the pass ends.
        disp = pcg.build_display_pcd(
            self.nonground_pcd, labels, n_clusters, palette, set(), False)
        evis = o3d.visualization.VisualizerWithEditing()
        evis.create_window(
            window_name="Selection Mode — shift+click cluster(s), click again "
                        "to toggle off, Q to confirm",
            width=1280, height=720)
        evis.add_geometry(disp)
        print("[select] Shift+click a cluster to select it (shift+click the "
              "same cluster again to deselect). Press Q when done.")
        evis.run()                 # blocks until Q/Esc — proven pattern
        evis.destroy_window()

        picked = evis.get_picked_points()
        selected_cids = set()
        for idx in picked:
            cid = int(labels[int(idx)])
            if cid == -1:
                continue            # noise point, ignore
            selected_cids.symmetric_difference_update({cid})  # toggle

        if selected_cids:
            print(f"[select] Selected cluster(s): {sorted(selected_cids)}. "
                  f"Showing highlighted preview — press Q to continue.")
            preview = pcg.build_display_pcd(
                self.nonground_pcd, labels, n_clusters, palette, selected_cids, True)
            pvis = o3d.visualization.Visualizer()
            pvis.create_window(
                window_name="Preview — selected cluster(s) highlighted. Press Q to continue.",
                width=1280, height=720)
            pvis.add_geometry(preview)
            pvis.run()              # blocks until Q/Esc
            pvis.destroy_window()
        else:
            print("[select] No clusters were selected.")

        self._confirm_selection(selected_cids, labels)

    def _confirm_selection(self, selected_cids, labels):
        self.mode = "normal"
        if not selected_cids:
            print("[select] Nothing selected — back to normal viewer.")
            self._open_normal_window()
            return

        room_pcd, obj_pcd, obj_mask = split_room_and_objects(
            self.nonground_pcd, labels, selected_cids)
        obj_src_idx = self.nonground_src_idx[obj_mask]

        # ── optional manual refinement (requirement 5) ──────────────────
        try:
            refine = messagebox.askyesno(
                "Refine selection?",
                "DBSCAN sometimes merges multiple objects into one cluster.\n\n"
                "Manually remove unwanted points from the selection?")
        except Exception:
            refine = False   # no Tk root available — skip refinement silently
        if refine:
            obj_pcd, obj_src_idx = self._manual_refine_object(obj_pcd, obj_src_idx)

        # Room = everything not in the object, minus what refinement carved
        # out (refinement only removes points FROM the object, so those
        # points still belong in the room — recompute room by set-difference
        # on indices rather than reusing the pre-refine room_pcd/obj split).
        keep_room_mask = ~np.isin(self.nonground_src_idx, obj_src_idx)
        room_nonground_idx = self.nonground_src_idx[keep_room_mask]
        room_pts  = self.nonground_pcd.select_by_index(np.where(keep_room_mask)[0])

        self.room_src_idx = np.concatenate([room_nonground_idx, self.ground_src_idx])
        self.room_pcd = _merge_pcds(room_pts, self.ground_pcd)

        print(f"Object: {len(obj_pcd.points):,} pts from {len(selected_cids)} "
              f"cluster(s). Room + floor: {len(self.room_pcd.points):,} pts.")

        # ── same hand-off the old on_continue() did ──────────────────────
        orig_attrs = None
        if self.src is not None and self.src.has_splat_attrs:
            orig_attrs = dict(
                scale_raw=self.src.scale_raw[obj_src_idx],
                rot_raw=self.src.rot_raw[obj_src_idx],
                opacity_raw=self.src.opacity_raw[obj_src_idx],
                f_dc=self.src.f_dc[obj_src_idx],
                f_rest=self.src.f_rest[obj_src_idx],
            )

        self.job = SplatJob()
        self.obj_added = False
        self.obj_geom = None
        threading.Thread(target=self.job.run, args=(obj_pcd,),
                          kwargs=dict(orig_attrs=orig_attrs), daemon=True).start()

        self._open_normal_window()   # restores original colors / removes clustering view

    def _manual_refine_object(self, obj_pcd, obj_src_idx):
        """Requirement 5: operates ONLY on obj_pcd's own points (not the
        whole scene). Shift+click a point to mark it for removal from the
        object; press Q to confirm the refined mask."""
        print("[refine] Shift+click points to REMOVE from the object "
              "(e.g. a neighboring object DBSCAN merged in). Press Q when done.")
        evis = o3d.visualization.VisualizerWithEditing()
        evis.create_window(window_name="Manual Refinement — shift+click points to remove, Q to confirm",
                            width=1280, height=720)
        evis.add_geometry(obj_pcd)
        evis.run()
        evis.destroy_window()

        remove_idx = set(evis.get_picked_points())
        if not remove_idx:
            print("[refine] Nothing marked for removal — keeping full selection.")
            return obj_pcd, obj_src_idx

        keep_mask = np.ones(len(obj_pcd.points), dtype=bool)
        keep_mask[list(remove_idx)] = False
        refined_pcd = obj_pcd.select_by_index(np.where(keep_mask)[0])
        refined_src_idx = obj_src_idx[keep_mask]
        print(f"[refine] Removed {len(remove_idx):,} points — "
              f"{len(refined_src_idx):,} remain in the object.")
        return refined_pcd, refined_src_idx

    # ── inserting the object once the background fit completes ─────────────
    def try_attach_object(self):
        if self.job is None or self.obj_added or not self.job.done.is_set():
            return
        self.obj_added = True
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                # Handled here — AFTER poll_events()/update_renderer() have
                # both returned for this frame — never inside the "S" key
                # callback itself. self.vis is reassigned to a new window
                # inside this call; that's only safe once we're outside the
                # old window's own event-polling call stack.
                if self._selection_requested:
                    self._selection_requested = False
                    self._run_selection_workflow()
        finally:
            self.vis.destroy_window()
...
def main():
    ...
    # Ground is still separated up front (cheap, and clustering should never
    # include the floor) — but DBSCAN itself is now deferred until the user
    # actually presses "S", inside CombinedScene, instead of running here
    # before the viewer has even opened (requirement 1 + faster startup).
    pcd = pcg.remove_ground(pcd_full)
    ground_mask = _extract_ground_mask(pcd_full, pcd)
    ground_src_idx = np.where(ground_mask)[0]
    nonground_src_idx = map_by_nearest(src.xyz, np.asarray(pcd.points))
    ground_pcd = _extract_ground(pcd_full, pcd)
    print(f"[ground] {len(ground_src_idx):,} floor points will be restored to the scene.")

    # Whole scene, nothing extracted yet — this is what "immediately open
    # the normal point cloud viewer" (requirement 1) means concretely.
    full_room_pcd = _merge_pcds(pcd, ground_pcd)
    full_room_src_idx = np.concatenate([nonground_src_idx, ground_src_idx])

    print("Room loaded. Press 'S' in the viewer to select an object.")
    scene = CombinedScene(
        full_room_pcd, job=None, src=src, room_src_idx=full_room_src_idx,
        nonground_pcd=pcd, nonground_src_idx=nonground_src_idx,
        ground_pcd=ground_pcd, ground_src_idx=ground_src_idx)
    scene.run()
