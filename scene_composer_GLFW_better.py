...
def main():
    ...
    # remove_ground() returns only the non-ground points. We now recover
    # BOTH a ground mask (over pcd_full / src's original row order) AND an
    # exact index mapping from the returned non-ground cloud back into
    # `src`, via nearest-neighbor matching. That mapping is what lets the
    # room ultimately get written back with ORIGINAL attributes instead of
    # synthetic ones — see room_src_idx below and its use in on_continue().
    pcd = pcg.remove_ground(pcd_full)
    ground_mask = _extract_ground_mask(pcd_full, pcd)          # over src rows
    ground_src_idx = np.where(ground_mask)[0]
    nonground_src_idx = map_by_nearest(src.xyz, np.asarray(pcd.points))  # pcd row -> src row
    ground_pcd = _extract_ground(pcd_full, pcd)                # kept for viewer merge only
